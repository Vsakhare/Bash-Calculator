	...
	while(1)
	{
		...
		else if(t.type == OPERATOR)
        {
            ...
            else if(t.op==')')
            {
                ...
                chpop(&char_st);         // for popping the '(' opening bracket
            }
            else
            {
...
int main()
{
    ...
    printf("Enter the expression and press ENTER to get result.......\n");
	number *r;
	char expr[3000];
	while(read(expr, 3000))
	{
		int i;
        for(i = 0; i < strlen(expr); i++) //Whenever '(' bracket occurs and after it norwritten as +3534....or -2525235....
